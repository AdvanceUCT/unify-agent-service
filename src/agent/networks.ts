 ...
 *
 * TODO(AD-68 / ledger owner):
 * If `/api/status` reports `ledger.reachable: false` with a pool timeout,
 * start here before changing service logic. Confirm that these genesis
 * transactions match the current BCovrin Test validator set and include the
 * full node list expected by Indy VDR. A stale or partial genesis file can
 * make the agent look broken even when the Docker container, wallet, and REST
 * API are healthy. Completion proof for AD-68 should be a fresh container
 * where `GET /api/status` returns `status: "ok"` and
